...
from multiprocessing import Queue  # Add this line
...
class Game:
    def __init__(self, player1, player2, time):
        ...
        self.c.bind("<Button-1>", self.on_canvas_click)
        self.c.pack()
    ...
    def on_canvas_click(self, event):
        if not self.game_over and self.players[self.current_turn].type == 'human':
            column = event.x // 100
            current_player = self.players[self.current_turn]

            if 0 in self.board[:, column]:  # Check if the column is not full
                self.update_board(int(column), current_player.player_number)
                ...
                if self.game_completed(current_player.player_number):
                    ...
                    # Automatically make AI or random move if the next player is not human
    ...
    def game_completed(self, player_num):
        ...
        def check_diagonal(b):
            for op in [None, np.fliplr]:
                ...
                root_diag = np.diagonal(op_board, offset=0).astype(np.integer)
                ...
                for i in range(1, b.shape[1]-3):
                    for offset in [i, -i]:
                        ...
                        diag = to_str(diag.astype(np.integer))
